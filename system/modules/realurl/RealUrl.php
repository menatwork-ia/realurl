...
class RealUrl extends Backend
{
    ...
    public function validateRegexp($strRegexp, $varValue, Widget $objWidget)
    {
        if ($strRegexp == 'folderurl') {
            ...
            if (preg_match('#/' . implode('/|/', $GLOBALS['URL_KEYWORDS']) . '/|/' . implode('$|/',
                    $GLOBALS['URL_KEYWORDS']) . '$#', $varValue, $match)) {
                $strError = str_replace('/', '', $match[0]);
                $objWidget->addError(sprintf($GLOBALS['TL_LANG']['ERR']['realurl'], $strError,
                    implode(', ', $GLOBALS['URL_KEYWORDS'])));
            }
                ...
                if ($mixAlias != false) {
                    ...
                    foreach ($arrPages as $subValue) {
                        ...
                        $mixAlias = $this->generateFolderAlias($subValue, '', false, true,
                            $objRootPages->realurl_force_alias);
     ...
     * @version 2.0
     */
    public function generateFolderAlias(
        $intID,
        $varValue = '',
        $blnCheckInput = false,
        $useExtException = false,
        $blnForce = false
    ) {
        // Init some Vars
        ...
        if (in_array($varValue, $GLOBALS['URL_KEYWORDS']) && $useExtException == false) {
            ...
            throw new Exception(sprintf($GLOBALS['TL_LANG']['ERR']['realUrlKeywordsExt'], $strUrl, $objPage->title,
                $objPage->id, $varValue), $objPage->id);
        }
